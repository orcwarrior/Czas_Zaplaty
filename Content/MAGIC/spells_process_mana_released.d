...
// Spells, die beim loslassen der Taste auch dann gecastet werden wenn noch nicht max. Mana investiert
	...
	if (Npc_GetActiveSpell(self) == SPL_MFIREBALL		)	{	return	SPL_SENDCAST;	};
...
// des Spells aufgerufen wird! Der Abzug von Schaden beim Opfer muß also im zs_"Victim"-Script des Opfers erfolgen...
	...
	if (Npc_GetActiveSpell	(self) == SPL_HEALOTHER			)	{	return	SPL_SENDSTOP;	};
...
// Spells, die abgebrochen werden, wenn beim loslassen der Taste noch nicht gecastet 
	...
	//	B_ChangeAttribute	(self,ATR_MANA,self.attribute[ATR_MANA]+manainvested);	
	...
	if (Npc_GetActiveSpell(self) == SPL_LIGHTNING_BIG	)	{	return	SPL_SENDSTOP; 	};
	if (Npc_GetActiveSpell(self) == SPL_THUNDERBOLT		)	{	return	SPL_SENDSTOP;	};
	if (Npc_GetActiveSpell(self) == SPL_Summong³upek	)	{	return	SPL_SENDSTOP;	};	
	...
	if (Npc_GetActiveSpell(self) == SPL_CreatePortal	)	{	return	SPL_SENDSTOP;	};
	if (Npc_GetActiveSpell(self) == SPL_GotoPortal		)	{	return	SPL_SENDSTOP;	};
	...
	if (Npc_GetActiveSpell(self) == SPL_LifeDrain			)	{	return	SPL_SENDSTOP;	};
	if (Npc_GetActiveSpell(self) == SPL_StuningBall			)	{	return	SPL_SENDSTOP;	};
	
	if (Npc_GetActiveSpell(self) == SPL_Meteor			)	{	return	SPL_SENDSTOP;	};	
	if (Npc_GetActiveSpell(self) == SPL_HolyFire			)	{	return	SPL_SENDSTOP;	};	
	
	if (Npc_GetActiveSpell(self) == SPL_FIREWALL				)	{	return	SPL_SENDSTOP;	};
	if (Npc_GetActiveSpell(self) == SPL_UNDRESS			)	{	return	SPL_SENDSTOP;	};
