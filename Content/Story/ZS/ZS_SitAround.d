...
{
	//PrintDebugInt			(PD_TA_FRAME,	"ZS_SitAround");
	
	B_SetPerception			(self);
	//--------- Mis Bukhart Hack ---------
	if(self.id==4021)
	{
		Burhart_Doing_Bad=FALSE;
	};
	//-------- Grobpositionierung --------
	if (Npc_GetDistToWP	(self,self.wp) > PERC_DIST_FLEE)
	{
	    //PrintDebugInt	(PD_TA_CHECK,	"...zu weit weg vom TA-Startpunkt!");
		AI_StandUp		(self);
...
func int ZS_SitAround_Loop ()
{
	...
	if (!C_BodyStateContains(self, BS_SIT))
	{
		...
		{
			//PrintDebugInt	(PD_TA_CHECK,	"...keine Sitzgelegenheit gefunden!");
	...
	else
	{
		...
		{
			if (Npc_GetAivar(self,AIV_HangAroundStatus) == 3)
			{
				AI_PlayAniBS (self, "R_CHAIR_RANDOM_1",BS_SIT);
			}
			else if (Npc_GetAivar(self,AIV_HangAroundStatus) == 2)
			{
...
{
	//PrintDebugInt (PD_TA_FRAME,"ZS_SitAround_End");
	
	if (Npc_GetAivar(self,AIV_HangAroundStatus) == 1)
	{
		AI_PlayAni (self, "T_SIT_2_STAND");
		Npc_SetAivar(self,AIV_HangAroundStatus,  0);
	}
	else if (Npc_GetAivar(self,AIV_HangAroundStatus) == 4)
	{
	...
	else if (Npc_GetAivar(self,AIV_HangAroundStatus) == 3)
	{
