...
{	
	...
	TEXT[5]				= NAME_Value;
	COUNT[5]			= value;
	on_state[0]				=	UseRESPAWNBOOK;
};

var int PP_BLACK;
var int PP_WOOD;

FUNC VOID UseRESPAWNBOOK()
{   
	PP_BLACK = 	Doc_Create		()			  ;								// DocManager 
					Doc_SetPages	( PP_BLACK,  1 );                         //wieviel Pages

					Doc_SetPage 	( PP_BLACK,  0, "PP_BLACK.tga" 	, 1		); 
	...
	nDocID = 	Doc_Create		()			  ;								// DocManager 
		...
		var c_npc v;
	v=Npc_GetNewsVictim(timedatt_memory,Npc_HasNews(timedatt_memory,500,NULL,NULL));				
					//1.Seite
  	var string str;
					Doc_SetMargins	( nDocID,  0,  275, 20, 30, 20, 1);  //  0 -> margins are in pixels				
					Doc_SetFont 	( nDocID, -1, "font_10_book.tga");													
   	str = ConcatStrings(concatstrings(concatstrings(v.name," 500."),inttostring(v.spawndelay)),v.wp);	
	Doc_PrintLines	( nDocID,  0, str);   
	v=Npc_GetNewsVictim(timedatt_memory,Npc_HasNews(timedatt_memory,499,NULL,NULL));				
   	str = ConcatStrings(concatstrings(concatstrings(v.name," 499."),inttostring(v.spawndelay)),v.wp);	
	Doc_PrintLines	( nDocID,  0, str);  
	v=Npc_GetNewsVictim(timedatt_memory,Npc_HasNews(timedatt_memory,498,NULL,NULL));	   
   	str = ConcatStrings(concatstrings(concatstrings(v.name," 498."),inttostring(v.spawndelay)),v.wp);	
	Doc_PrintLines	( nDocID,  0, str);  
	v=Npc_GetNewsVictim(timedatt_memory,Npc_HasNews(timedatt_memory,497,NULL,NULL));	     
   	str = ConcatStrings(concatstrings(concatstrings(v.name," 497."),inttostring(v.spawndelay)),v.wp);	
	Doc_PrintLines	( nDocID,  0, str);  
	v=Npc_GetNewsVictim(timedatt_memory,Npc_HasNews(timedatt_memory,496,NULL,NULL));	     
   	str = ConcatStrings(concatstrings(concatstrings(v.name," 496."),inttostring(v.spawndelay)),v.wp);	
	Doc_PrintLines	( nDocID,  0, str);  
	v=Npc_GetNewsVictim(timedatt_memory,Npc_HasNews(timedatt_memory,495,NULL,NULL));	     
   	str = ConcatStrings(concatstrings(concatstrings(v.name," 495."),inttostring(v.spawndelay)),v.wp);	
	Doc_PrintLines	( nDocID,  0, str);    
	v=Npc_GetNewsVictim(timedatt_memory,Npc_HasNews(timedatt_memory,494,NULL,NULL));	     
   	str = ConcatStrings(concatstrings(concatstrings(v.name," 494."),inttostring(v.spawndelay)),v.wp);	
	Doc_PrintLines	( nDocID,  0, str);        												
...
{	
	...
	TEXT[5]				= NAME_Value;
	COUNT[5]			= value;
	on_state[0]				=	UseART_STORYBOOK;
};
	
FUNC VOID UseART_STORYBOOK()
{   
	Doc_SetPages	( PP_BLACK,  -1 );                         //Remove PP EFFECT
				
	//Doc_SetPage 	( PP_BLACK,  0, "SIMPLEGLOW.tga" 	, 0		); 
	return;
		var int nDocID;
	nDocID = 	Doc_Create		()			  ;								// DocManager 
					Doc_SetPages	( nDocID,  2 );                         //wieviel Pages
					...
					Doc_PrintLines	( nDocID,  0, "Wyruszyliœmy z misj¹ niezwyk³ej wagi. Wed³ug starych manuskryptów w g³êbinach tych mrocznych jaskiñ spoczywa artefakt, który ca³kowicie odmieni³by nasze nêdzne ¿ycie... Jesteœmy na miejscu. Obiekt naszych marzeñ jest ju¿ na wyci¹gniêcie rêki. Niestety dziel¹ nas od niego masywne, ¿elazne kraty których nie sposób sforsowaæ. Musi byæ jakiœ inny sposób dostania siê na drug¹ stronê...");
			///		Doc_PrintLines	( nDocID,  0, "Die Fähigkeiten die göttliche Macht zu kanalisieren wächst in dem Magier. Anfangs keimt sie nur wie ein kleiner Schössling auf dem Feld und man muss ihn pflegen, damit er nicht verkümmert. Doch wenn dieser Schössling erst einmal herangewachsen ist, entfaltet er seine volle Pracht");
					...
					Doc_SetFont 	( nDocID, -1, "font_10_book.tga");								
					Doc_PrintLine	( nDocID,  1, "Dzieñ 2");			
					Doc_PrintLine	( nDocID,  1, "");
					Doc_PrintLines	( nDocID,  1, "Na œcianie znaleŸliœmy dziwne przyciski. Marwez i Renor próbowali je wcisn¹æ, ale ...Zw³oki zakopaliœmy na zewn¹trz. Dziêki ich ofierze  dowiedzieliœmy siê jaki przycisk nale¿y wcisn¹æ. Odsunê³a siê jedna ze œcian, a my znaleŸliœmy stary ornament. Da³em go na przechowanie Patrikowi po czym ruszyliœmy dalej. Na koñcu ciemnego korytarza znalaz³em wy¿³obienie w œcianie, które idealnie pasowa³o do kszta³tu ornamentu. Zawo³a³em Patrika jednak¿e nie odpowiedzia³ mi ¿aden g³os. Ten drañ zwia³ z moim ornamentem! Za wszelk¹ cenê muszê go odnaleŸæ!");
					Doc_Show		( nDocID );
	};

func void HUN_CreateInfoBook()
{
/* var string msgguid;
var string msghun1;
var string msghun2;
...
var string msg;
msg = ConcatStrings("Dowódca to " , guiderNAME ));
msg = ConcatStrings(msg , " Jego poziom to: "));
msg = ConcatStrings(msg,IntToString(guider.level));
msg = ConcatStrings(msg , " Posiada Miecz: ");
msg = ConcatStrings(msg,IntToString(Npc_GetEquippedMeleeWeapon(guider).name));
//guider
if (Hlp_IsValidNpc(hunter1))
{
msg = ConcatStrings("Pierwszy Myœliwy ",hunterNAME1));
msg = ConcatStrings(msg , " Jego poziom to:"));
msg = ConcatStrings(msg , IntToString(hunter1.level));
msg = ConcatStrings(msg , " Posiada Miecz: ");
msg = ConcatStrings(msg , IntToString(Npc_GetEquippedMeleeWeapon(hunter1).name));
};
if (Hlp_IsValidNpc(hunter2))
{
msg = ConcatStrings("Drugi Myœliwy ",hunterNAME1));
msg = ConcatStrings(msg , " Jego poziom to:"));
msg = ConcatStrings(msg , IntToString(hunter2.level));
msg = ConcatStrings(msg , " Posiada Miecz: ");
msg = ConcatStrings(msg , IntToString(Npc_GetEquippedMeleeWeapon(hunter2).name));
};
...
/******************************************************************************************/
INSTANCE HunBook(C_Item)
{	
	name 					=	"Menu: informacje o polowaniu";
	...
	TEXT[5]				= NAME_Value;
	COUNT[5]			= value;
	on_state[0]			=	UseHunBook;
   description = name;
};

FUNC VOID UseHunBook()
{   
	HUN_CreateInfoBook();
};
/******************************************************************************************/
...
{	
	...
	material 			=	MAT_LEATHER;
	on_state[0]			=   UsegBrief;
	scemeName			=	"MAP";
	description			= "Jakiœ list";
};
...
{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_SetFont 	( nDocID, -1, "font_10_book.tga"	   			); 	// -1 -> all pages 
					Doc_SetMargins	( nDocID, -1, 20, 50, 10, 20, 2   		);  //  0 -> margins are in pixels
					Doc_PrintLine	( nDocID,  0, " "					);
					Doc_PrintLine	( nDocID,  0, "Otrzymujecie rozkaz atakowania wszystkich:"		);
					Doc_PrintLine	( nDocID,  0, " "				);
					Doc_PrintLine	( nDocID,  0, "Szkodników,Najemników,Nowicjuszy,Guru i Templariuszy"				);
					Doc_PrintLine	( nDocID,  0, "Których bêdziecie mieli w polu widzenia"					);
					Doc_SetFont 	( nDocID, -1, "font_10_book.TGA"	   			); 	// -1 -> all pages 
					Doc_PrintLines	( nDocID,  0, "W Najbliszym tygodniu rozkaz wprowadzimy w starym obozie"	);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, "a tego konsekwêcj¹ bedzie morderstwo wszyskich powy¿ej wymienionych w Starym obozie"					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, "Najwazniejsz¹ rzecz¹ jest abyœcie nie pozwolili nikomu spoza starego obozu przeczytaæ tego pisma"					);
					Doc_PrintLine	( nDocID,  0, "Niech wasza wiara wpomaga moc");
					Doc_PrintLine	( nDocID,  0, "A moc niech wspomaga wiarê"	);
					Doc_PrintLine	( nDocID,  0, ""	);
					Doc_PrintLine	( nDocID,  0, ""				);
					Doc_PrintLine	( nDocID,  0, "Najwyzszy z magnatów"					);
					Doc_PrintLine	( nDocID,  0, "Gomez"					);
					Doc_SetMargins	( nDocID, -1, 20, 10, 50, 20,2   		);  //  0 -> margins are in pixels (Position des Textes von den Ränder des TGAs aus
...
INSTANCE Nadzorca_Brief (C_Item)
{	
	name 				=	"List pisany niepewn¹ rêk¹";
	...
	material 			=	MAT_LEATHER;
	on_state[0]			=   UsemBrief;
	scemeName			=	"MAP";
	description			= "Temu kto pisa³ ten list bardzo siê spieszy³o";
};
func void UsemBrief ()
{   
		var int nDocID;
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_SetMargins	( nDocID, -1, 20, 50, 10, 20, 2   		);  //  0 -> margins are in pixels
					Doc_PrintLine	( nDocID,  0, " "					);
					Doc_PrintLine	( nDocID,  0, "Wszyscy skazancy uwa¿ajcie na Gomeza i jego stra¿ników:"		);
					Doc_PrintLine	( nDocID,  0, " "				);
					Doc_PrintLine	( nDocID,  0, "Kiedy jego stra¿nicy atakowali mnie,mojego doradce Macros'a i mych dwóch obobistych stra¿ników"				);
					Doc_PrintLine	( nDocID,  0, "Straznicy zabijajac jednego z mych stra¿ników mówili ¿e to samo w krótce stanie siê z wszystkimi zdrajcami starego obozu"					);
					Doc_SetFont 	( nDocID, -1, "font_10_book.TGA"	   			); 	// -1 -> all pages 
					Doc_PrintLines	( nDocID,  0, ""	);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, "Wiêc strzezcie siê Gomeza i jego Stra¿ników"					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, "Wiêc spoztrzeœcie nasz b³¹d i go nie powtórzcie");
					Doc_PrintLine	( nDocID,  0, "Nadzorca górniczej doliny"	);
					Doc_PrintLine	( nDocID,  0, ""	);
					Doc_PrintLine	( nDocID,  0, ""				);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, "Juren"					);
					Doc_SetMargins	( nDocID, -1, 20, 5, 30, 20,2   		);  //  0 -> margins are in pixels (Position des Textes von den Ränder des TGAs aus
...
{	
	...
	visual 				=	"ItWr_Scroll_01.3DS";
	material 			=	MAT_LEATHER;
	on_state[0]			=   Usedeep_letter1;
	scemeName			=	"MAPSEALED";
	description			= "List";
	TEXT[0]				= "Po piœmie widaæ, ¿e autor tego listu";
	TEXT[1]				= "by³ mocno wstawiony.";
};
func void Usedeep_letter1 ()
{   
	var int nDocID;
	...
	nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, "Mam ju¿ doœæ tego cholernego miejsca!");
					Doc_PrintLine	( nDocID,  0, "Mog³em daæ siê zabiæ rebeliantom,");
               Doc_PrintLine	( nDocID,  0, "a nie gniæ tutaj i czekaæ na œmieræ.");
               Doc_PrintLine	( nDocID,  0, "St¹d nie ma ucieczki. Ka¿dy kto tu");
					Doc_PrintLine	( nDocID,  0, "trafi, zostanie tu na wieki.");
               Doc_PrintLine	( nDocID,  0, "Podobno ten ca³y Paj¹k wie, jak st¹d uciec.");
               Doc_PrintLine	( nDocID,  0, "Niestety, temu idiocie odbi³o. Mnie te¿"); 
               Doc_PrintLine	( nDocID,  0, "powoli zaczyna odbijaæ. S³yszê jakieœ");
               Doc_PrintLine	( nDocID,  0, "szepty w mojej g³owie. Te cholerne g³osy");
               Doc_PrintLine	( nDocID,  0, "drêcz¹ mnie ca³y czas! Jedynie dobre");
					Doc_PrintLine	( nDocID,  0, "piwo mnie uspokaja i powoduje, ¿e nie myœlê");
					Doc_PrintLine	( nDocID,  0, "o tym zasranym miejscu i nie s³yszê");
					Doc_PrintLine	( nDocID,  0, "tych g³osów. Zostawcie mnie w spokoju!");
					Doc_PrintLine	( nDocID,  0, "W dupie was mam! I tych idiotów te¿.");
               ...
               Doc_PrintLine	( nDocID,  0, "mu do tej t³ustej dupy, ale goœæ jest za dobry.");
					Doc_PrintLine	( nDocID,  0, "¯egnaj okrutny œwiecie! Utopiê siê w piwie");
					Doc_PrintLine	( nDocID,  0, "i po problemie. Wara od mojej skrzyni!");
					Doc_PrintLine	( nDocID,  0, "Chocia¿ i tak jeden z tych œmierdz¹cych");
					Doc_PrintLine	( nDocID,  0, "kopaczy ukrad³ do niej klucz...");
