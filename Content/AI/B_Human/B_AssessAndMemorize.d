...
//punishment-system - used only in Rebel-Camp
var int	GOLDPENALTY_THEFT;
var int	GOLDPENALTY_DEFEAT;
var int	GOLDPENALTY_MURDER;
...
// vict			:	Opfer
{
	////PrintDebugInt( PD_ZS_CHECK,"B_AssessAndMemorize");
	...
	{
		////PrintDebugInt	(PD_ZS_CHECK,"...Täter, Zeuge oder Opfer KEIN Mensch!");
	...
	{
		////PrintDebugInt( PD_ZS_CHECK,"...AssessAndMemMurder");
		Npc_MemoryEntry (witness,source,offender,newsid,vict ); // Und News merken hier sollte eigentlich Gilde als Opfer, es ist aber  
															 ...
															 //	des Nsc zuzugreifen
		//punishment?
		if(Npc_IsPlayer(offender))&&((Npc_CanSeeNpcFreeLos(witness,vict))||Npc_CanSeeNpcFreeLos(witness,offender))&&(C_NpcBelongsToRebelCamp(vict))&&(C_NpcBelongsToRebelCamp(witness))
		&&(C_GetAttackReason(vict)!=AIV_AR_PERSONALAFFAIRS)
		{
			GOLDPENALTY_MURDER=GOLDPENALTY_MURDER+200;	
			B_PrintDebug(concatstrings("murder_gold=",inttostring(GOLDPENALTY_MURDER)));
		};														 
		if (Wld_GetGuildAttitude (witnessguild, victimguild) == ATT_FRIENDLY)
		{
			////PrintDebugInt( PD_ZS_CHECK,"...AssessAndMemMurder Knows Player // friendly victim");
			if (Npc_IsPlayer (offender)) 
			{
				////PrintDebugInt( PD_ZS_CHECK,"...AssessAndMemMurder Knows Player // friendly victim // Player attacks");
				if (Npc_GetPermAttitude (witness,offender) == ATT_ANGRY)
				{
					////PrintDebugInt( PD_ZS_CHECK,"...AssessAndMemMurder Knows Player // friendly victim // Player attacks // Angry to Plyaer");
				...
				{
					////PrintDebugInt( PD_ZS_CHECK,"...AssessAndMemMurder Knows Player // friendly victim // Player attacks // Friend/Neut to Player");
	...
	{
		////PrintDebugInt( PD_ZS_CHECK,"...AssessAndMemTheft");
		// Bin ich das Opfer ?
		...
		{
			if(Npc_IsPlayer(offender))&&((Npc_CanSeeNpcFreeLos(witness,vict))||Npc_CanSeeNpcFreeLos(witness,offender))&&(C_NpcBelongsToRebelCamp(witness))
			{
				if(item.product_amount!=47)//TODO: IT CAN CAUSE BUGS
				{
				item.product_amount=47;	
				GOLDPENALTY_THEFT=GOLDPENALTY_THEFT+item.value;	
				B_PrintDebug("item_val_added");
				}
				else
				{
				GOLDPENALTY_THEFT=GOLDPENALTY_THEFT+50;		
				};												 
				B_PrintDebug(concatstrings("theft_gold=",inttostring(GOLDPENALTY_THEFT)));
			};			
			if (B_CompareNpcInstance ( witness,vict ))
			{
				////PrintDebugInt( PD_ZS_CHECK,"...AssessAndMemTheft Someone stole me something");
				Npc_MemoryEntry (witness, source, offender, newsid, vict);
				if (Npc_GetPermAttitude(witness, offender) != ATT_HOSTILE)
				{
					////PrintDebugInt( PD_ZS_CHECK,"...AssessAndMemTheft Someone stole me something and im not hostile");
					if (Npc_IsPlayer(offender))
					{
						////PrintDebugInt( PD_ZS_CHECK,"...AssessAndMemTheft Player stole me something");
		...
		{
			////PrintDebugInt( PD_ZS_CHECK,"...AssessAndMemTheft Someone stole another something");
			if (((Wld_GetGuildAttitude(witnessguild, ownerguild) == ATT_FRIENDLY) ||( witnessguild == ownerguild)) && (ownerguild != offenderguild))
			{
				////PrintDebugInt( PD_ZS_CHECK,"...AssessAndMemTheft Someone(wrong guild) stole another (friend or my guild) something");
				if (Npc_IsPlayer (offender))
				{
					Npc_MemoryEntry (witness, source, offender, newsid, vict);
					Npc_SetTempAttitude (witness, ATT_HOSTILE);
	...
	{
		////PrintDebugInt( PD_ZS_CHECK,"...NEWS_DEFEAT");
		// Jp: alte abfrage raus, weil sie doppelt war
		//if (Npc_IsPlayer	( offender) || Npc_IsPlayer	( vict))
		if(Npc_IsPlayer(offender))&&((Npc_CanSeeNpcFreeLos(witness,vict))||Npc_CanSeeNpcFreeLos(witness,offender))&&(C_NpcBelongsToRebelCamp(vict))&&(C_NpcBelongsToRebelCamp(witness))
		{
			if(CmpNpc(witness,vict))&&(C_GetAttackReason(vict)!=AIV_AR_PERSONALAFFAIRS)
			&&(C_NpcBelongsToRebelCamp(vict))&&(C_NpcBelongsToRebelCamp(witness))
			{
			GOLDPENALTY_DEFEAT=GOLDPENALTY_DEFEAT+75;	
			}
			else if(C_GetAttackReason(vict)!=AIV_AR_PERSONALAFFAIRS)
			&&(C_NpcBelongsToRebelCamp(vict))&&(C_NpcBelongsToRebelCamp(witness))
			{
			GOLDPENALTY_DEFEAT=GOLDPENALTY_DEFEAT+25;				
			};											 
		};			
		...
		if (Npc_IsPlayer (offender) )
		{
			////PrintDebugInt( PD_ZS_CHECK,"...Sieger ist SC!");
			//PrintGlobals	(PD_ZS_CHECK);
			if ((Wld_GetGuildAttitude (witnessguild,victimguild)==ATT_FRIENDLY) && (!B_CompareNpcInstance ( witness,vict)))
			{
				////PrintDebugInt( PD_ZS_CHECK,"...NSC ist in befreundeter Gilde zum Besiegten!");
				//PrintGlobals	(PD_ZS_CHECK);
				if (Npc_GetTempAttitude (witness,offender) == ATT_ANGRY && (!(C_NpcIsWorker	( self))))
				{
					////PrintDebugInt( PD_ZS_CHECK,"...permANGRY zum Sieger und weder Buddler, noch Novize, noch Bauer!");
					Npc_SetTempAttitude (witness,ATT_HOSTILE);
				}
				else if (Npc_GetTempAttitude (witness,offender) == ATT_ANGRY && (C_NpcIsWorker	( self))) 
				{
					////PrintDebugInt( PD_ZS_CHECK,"...AssessAndMemDefeat Player (Angry) is offender victim is my guild or friendly but I,m unimportant guild");
				...
				{
					////PrintDebugInt( PD_ZS_CHECK,"...AssessAndMemDefeat Player(neutral/friendly is offender victim is my guild or friendly");
					Npc_SetTempAttitude (witness,ATT_ANGRY);
				};
			}
			else if (B_CompareNpcInstance ( witness,vict))
			{
				////PrintDebugInt( PD_ZS_CHECK,"...AssessAndMemDefeat Player is offender victim is me");
				if ((C_NpcIsWorker	( self)) )
				{
					////PrintDebugInt( PD_ZS_CHECK,"...AssessAndMemDefeat Player is offender victim is me (weak guild)");
					Npc_SetTempAttitude (witness,ATT_ANGRY);
				}
				else if (witnessguild == GIL_MAG ||witnessguild ==  GIL_KDW)
				{
					////PrintDebugInt( PD_ZS_CHECK,"...AssessAndMemDefeat Player is offender victim is me ( mage guild)");
					Npc_SetTempAttitude (witness, ATT_HOSTILE);
				}
				else 
				{
					////PrintDebugInt( PD_ZS_CHECK,"...AssessAndMemDefeat Player is offender victim is me ( other guild)");
					if (Npc_GetTempAttitude (witness, offender) != ATT_HOSTILE )
					{
						////PrintDebugInt( PD_ZS_CHECK,"...AssessAndMemDefeat Player is offender victim is me ( other guild// not hostile)");
						Npc_SetTempAttitude (witness,ATT_ANGRY);
					};
				};
			};
