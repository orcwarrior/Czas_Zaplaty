		...
		if (B_CompareNpcInstance (murdernews_offender, other))
		{
			...
			if ( Npc_GetPermAttitude ( self, other) == ATT_FRIENDLY || Npc_GetPermAttitude ( self, other) == ATT_NEUTRAL)
			{
				...
				//	Wichtige Gilden (GURU, EBR, MAGE) haben eigenes SVM
					...
					if ( murdernews_victim.guild == GIL_HUN)
					{
					...
					if ( murdernews_victim.guild == GIL_KDW || murdernews_victim.guild == GIL_MAG)
					{
...
// 					if ( murdernews_victim.guild == GIL_RBL)
// 					{
...
// 					if ( murdernews_victim.guild == GIL_MTR )
// 					{
...
// 						return;
// 					};
// 					if ( murdernews_victim.guild == GIL_PIR )
// 					{
...
// 						return;
// 					};
// 					if ( murdernews_victim.guild == GIL_SLV)
// 					{
...
// 					};
				...
				//	Ein SVM für restl. Gilden Altes Lager
// 					if ( murdernews_victim.guild == GIL_PUP)
// 					{
...
// 					};
	...
	//-------------------------------------------ReactToAttackNews--------------------------------------------------------
	//Per aiv'ar noch einen Kommentar, bei Nsc selbst Attackopfer gewesen
	
	if (Npc_GetAivar(self,AIV_BEENATTACKED)	== TRUE)&&((self.flags & NPC_FLAG_FRIEND) == 0)
	{
	...
	if (defeat_news > 0)
	{
		...
		if (B_CompareNpcInstance ( other, defeat_news_offender))
		{
			...
			if (B_CompareNpcInstance ( self, defeat_news_victim))&&((self.flags & NPC_FLAG_FRIEND) == 0)
			{
			...
			else if (!B_CompareNpcInstance ( defeat_news_victim, other))	// anderer Nsc ist das Opfer 
			{
				...
				if (Npc_GetPermAttitude	( self, defeat_news_offender) == ATT_ANGRY )
				{
					...
					if( Wld_GetGuildAttitude ( self_guild, defeat_news_victim_guild) == ATT_FRIENDLY)
					{
						...
						if ( defeat_news_victim_guild == GIL_HUN)
						{
						...
						else if ( defeat_news_victim_guild	== GIL_MAG || defeat_news_victim_guild	== GIL_KDW)
						{
						...
						else if	(self_guild == GIL_GRD && ( defeat_news_victim_guild == GIL_VLK || defeat_news_victim_guild == GIL_RBL))
						{
...
// 						else if (self_guild == GIL_TPL	&& defeat_news_victim_guild	== GIL_PUP)
// 						{
...
// 						}
				...
				else if (Npc_GetPermAttitude	( self, defeat_news_offender) == ATT_FRIENDLY || Npc_GetPermAttitude	( self, defeat_news_offender) == ATT_NEUTRAL)
				{
					...
					if (defeat_news_victim_guild == GIL_HUN && self_guild != GIL_HUN)
					{
...
// 						return;
// 					}
					else if	( defeat_news_victim_guild	== GIL_KDW || defeat_news_victim_guild	== GIL_MAG)
					{
					...
					{
						if ((defeat_news_victim_guild == GIL_VLK || defeat_news_victim_guild == GIL_RBL) && self_guild == GIL_GRD)
						{
...
// 						if (defeat_news_victim_guild == GIL_PUP && self_guild == GIL_TPL)
// 						{
...
// 								};
		...
		else // SC ist Victim
		{
			...
			if (B_CompareNpcInstance	( self, defeat_news_offender))&&((self.flags & NPC_FLAG_FRIEND) == 0)
			{
			...
			else if((self.flags & NPC_FLAG_FRIEND) == 0)
			{
	...
	if (theft_news > 0 )
	{
		...
		if ( B_CompareNpcInstance ( other,theft_news_offender))
		{
			...
			if (Npc_GetPermAttitude	( self, other) == ATT_FRIENDLY || Npc_GetPermAttitude	( self, other) == ATT_NEUTRAL)
			{
				if ( theft_news_victim.guild == GIL_HUN && self.guild != GIL_HUN)
				{
...
// 					return;
// 				};
				if ( theft_news_victim.guild == GIL_KDW || theft_news_victim.guild == GIL_MAG)
				{
