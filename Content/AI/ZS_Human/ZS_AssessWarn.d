...
func void ZS_AssessWarn ()
{
	...
	if (Npc_GetAivar(victim,AIV_ITEMSCHWEIN)==TRUE)
	...
	&&		(Npc_GetAttitude(self,victim) == ATT_FRIENDLY)
	{
	    ...
	    if (Npc_GetAivar(other,AIV_HASBEENDEFEATEDINPORTALROOM) == FALSE)
		{
	...
	else if(self.guild==GIL_GRD)&&(Npc_GetDistToNpc(self,other) < HAI_DIST_HELPATTACKEDCHARGES)
	{ 	//SN: keine Default-Reaktion!!!
		//AI_Wait			(self,	1.0);		// Reaktionszeit
		//B_WhirlAround 	(self, 	other);
				B_PrintDebug(concatstrings(self.name,"-gonna attack u'"));

			B_FullStop(self);
			Npc_SetTarget	(self,	other);
			AI_StartState	(self,	ZS_Attack,	0,	"");		
//-pudlo, zrobi sie to przez assesmemory			
// 	}
// 	else if(C_NpcBelongsToRebelCamp(self))&&(Npc_CanSeeNpcFreeLOS(self,other))
// 	{
// 	witneses=witneses+1;
// 	B_PrintDebug(concatstrings(inttostring(witneses),"witnesses"));	
	};	
};
