...
{	
	...
	if (	Npc_GetAivar(self,AIV_WASDEFEATEDBYSC) 					||		// Wurde NSC schon mal besiegt, oder...
			(C_NpcIsWorker(self) && C_AmIWeaker(self, other))		)&&((self.flags & NPC_FLAG_FRIEND) == NPC_FLAG_FRIEND)	// ...ist NSC ein schwächerer WORKER ?
	{
...
{	
	...
	//Orc: Bugfix sometimes game dont get that weapon was removed
	if (Npc_IsInFightMode(other, FMODE_NONE))
	{
			return			LOOP_END;		
	}
	else if (Npc_GetDistToNpc(self,other) < HAI_DIST_ABORT_MELEE)
	{
		////PrintDebugNpc		(PD_ZS_CHECK,	"...SC ist in Nahkampfdistanz!");

		//---- Passender Kommentar ! ----
		if (Npc_GetAivar(self,AIV_FIGHTSPEACHFLAG)==0)
		{
			...
			Npc_SetAivar(self,AIV_FIGHTSPEACHFLAG,  1);
		};
