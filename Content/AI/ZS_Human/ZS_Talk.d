...
FUNC void ZS_Talk ()
{
	...
	var ocNPC talker; var oCNPC hiro;
	talker = Hlp_GetNpc(Hlp_GetInstanceID(self));
	hiro = Hlp_GetNpc(Hlp_GetInstanceID(other));
	if(C_AmIStronger(self,other))&&(Hlp_random(20) == 1)&&(Npc_GetAttitude(self,other) != ATT_FRIENDLY)
	{
		
	};
	//-------- Verhindern von Mehrfachansprechen der NSCs --------
	//-------- hero & npc height differs >150 --------------------
	var int diff; diff = subf(hiro._zCVob_trafoObjToWorld[7],talker._zCVob_trafoObjToWorld[7]);
	diff = absf(diff); diff = gef(diff,mkf(150));
	if ((Npc_GetAivar(other,AIV_INVINCIBLE)==TRUE)&&(!Hero_Important))
	|| (diff)
	{
	...
	Npc_SetAivar(other,AIV_INVINCIBLE,TRUE);
	...
	Npc_PercDisable   (self, PERC_NPCCOMMAND);//Important! Bugfix
	...
	&&	(Npc_GetAivar(self,AIV_TALKBEFOREATTACK) == FALSE)
	{
	...
	else if ( (!C_BodystateContains(self, BS_SIT)) || (!Npc_CanSeeNpc(self, hero)) )//wisp can't turn
	{
		AI_StandUp		(self);
		AI_TurnToNpc	(self,	hero);
	};
	...
	if (!Npc_GetAivar(hero,AIV_IMPORTANT))
	{
		B_GuildGreetings ();
	};
	...
	Npc_SetAivar(self,AIV_FINDABLE,  1); 
	
	
	Trade_InitDefaultMultipilers();
...
FUNC INT ZS_Talk_Loop ()
{
	...
	{
   		if (Npc_GetAivar(self,AIV_HANGAROUNDSTATUS) <= 1)&&(self.id != 2003)&&(self.id != 2001) //sitzt auf Boden, NICHT auf Bank, etc.
   		{
   	...
   	if ( InfoManager_HasFinished() )
   	{
		...
		Npc_SetAivar(other,AIV_INVINCIBLE,  FALSE);
...
{
	Npc_PercEnable		(self,	PERC_NPCCOMMAND			,	B_CheckSheath);//Important! Bugfix

	B_ThingsToDoAfterDialog(self);	
};
