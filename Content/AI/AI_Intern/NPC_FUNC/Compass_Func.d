...
func void COMPASS_TRIGGER()
{
	...
	{
		printdebug("Exit from CompassTrigger");
		return;	
	};
	//TMP trigger off
	//return;
	...
	//f = "Compass_Rose128.tga";  // compass rose (not included in the package)
 	if(COMPASS_INIT)
 	{
		printdebug("CompassTrigger<<COMPASS_INIT");
 		t = Compass_ForHero();
 	...
 	if(COMPASS_PTR==0)
	{
		...
		{
			printdebug("Exit from CompassTrigger<<!MEM_ReadInt(ptr+140)");
			COMPASS_INIT = true;
			return;	
		};
		if(ptr)
			...
			if(ptr)&&(MEM_ReadInt(ptr)==zCMenuItem_vtbl)
			{
				if(ptr)
				{
					...
					{//its compas text
						...
						printdebug("Exit from CompassTrigger<<Hlp_StrCmp(MEM_ReadString(MEM_ReadInt(ptr+32)),f)");
						return;//exit loop
					};  
				};		
			}
