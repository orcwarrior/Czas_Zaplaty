...
func int ZS_MM_SummonedByPC_Loop()
{
	...
	if (Npc_GetStateTime(self) > Npc_GetAivar(self,AIV_MM_TimeLooseHP))
	{
		//-------- Kreatur wird nach einiger Zeit schwächer !--------
		B_ChangeAttribute	(self, ATR_HITPOINTS, -1);
	...
	{
		//-------- SC-Meister folgen ! --------
		if (Npc_GetDistToNpc(self,hero) > Npc_GetAivar(self,AIV_MM_DistToMaster))
		{
...
func void B_SummonedByPC_AssessEnemy ()
{
	...
	if ( (!Npc_GetAivar(other,AIV_PARTYMEMBER)) && (other.npctype != NPCTYPE_FRIEND) )
	{
		AI_StartState(self,	ZS_MM_Attack,	0,	"");
	};
};
