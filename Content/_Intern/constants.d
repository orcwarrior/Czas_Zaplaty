...
CONST INT NPC_FLAG_BRAVE								=  1 << 3				;//Never run like a coward
CONST INT NPC_FLAG_KILLER								=  1 << 4				;//Npc kills enemies instantly																
CONST INT NPC_FLAG_TALKONLY								=  1 << 5				;//Only Perc_AssesTalk Perception enabled (pushed on any other disturbing perception too)															
//																				 
//	NPC TYPES																	 
//																				 
...
CONST INT DAM_INDEX_MAX									= DAM_INDEX_FALL	+ 1 ;//8

//																				 
//	OTHER DAMAGE CONSTANTS														 
//																				 
CONST INT NPC_ATTACK_FINISH_DISTANCE					= 180					;
CONST INT NPC_BURN_TICKS_PER_DAMAGE_POINT				= 80					;		
...
const int NPC_TALENT_PICKLOCK/*10*/	= 5;
const int NPC_TALENT_PICKPOCKET/*11*/= 6;
const int NPC_TALENT_EVASION		= 7;
// Special-Talents
const int NPC_TALENT_SMITH			= 8; //Sneak set skill 1 on acrobat var
const int NPC_TALENT_REGEN_HP/*5*/	= 9; //regeneration
//1=60str per 1hp/s 2=25str per 1/hps
//Cost:10,20LP
const int NPC_TALENT_REGEN_MP/*6*/	= 10;    //firemaster
const int NPC_TALENT_ALCHEMY/*9*/	= 11;	//Acrobat
...
const int NPC_TALENT_MAX			= 12;
 ...
 //updates                      **\\
//*********************************\\
const int HACK_NPC_TALENT_MAGE			= 7;	//normal(secondary skill) description: No(No)/Yes(No)/No(Yes)/Yes(Yes)
const int HACK_NPC_TALENT_SNEAK			= 2;	
const int HACK_NPC_TALENT_ACROBAT		= 3;
 ...
 //new                          **\\
//*********************************\\
const int HACK_NPC_TALENT_QUICK_LERNING	= 4;
//0=+0,1=+2,2=+3 
//Cost: 5LP,10LP
const int HACK_NPC_TALENT_RUNESWORDUSING= 5;
const int HACK_NPC_TALENT_DUALUSING     = 6;
const int HACK_NPC_TALENT_ITEM_CHARMER  = 7;
...
Tal_Configure(HACK_NPC_TALENT_MAGE,			0);
Tal_Configure(HACK_NPC_TALENT_SNEAK,			0);
Tal_Configure(NPC_TALENT_REGENERATE,	false);
Tal_Configure(NPC_TALENT_FIREMASTER,	0);
Tal_Configure(HACK_NPC_TALENT_ACROBAT,		0);
*/
...
CONST INT GIL_HUN										=  1					;		//	Erzbarone 6* +3Babes
CONST INT GIL_GRD										=  2					;		//	Söldner
CONST INT GIL_RBL										=  3					;		//	Rebel
CONST INT GIL_MAG										=  4					;		//	Kreis des Feuers 6
CONST INT GIL_VLK										=  5					;		//	Volk/Buddler 10
CONST INT GIL_KDW										=  6					;		//	Kreis des Wassers 6
CONST INT GIL_MTR										=  7					;		//	Mutineer = Rebel in New Camp
CONST INT GIL_PIR										=  8					;		//	Pirat
CONST INT GIL_SLV										=  9					;		//	Slave in NO
CONST INT GIL_SFB										= 10					;		//	Schürferbund 9-10 svm
CONST INT GIL_GUR										= 11					;		//	Free?
CONST INT GIL_PUP										= 12					;		//	Guard in NO (on Revolt with MTR, anti GRD)
CONST INT GIL_TPL										= 13					;		//	Skeleton Archers
...
CONST INT GIL_PUPIL										= 18					;		//	Schläfer
...
//	MOBSI DIALOGS
//
// - 21-30 stylist dialogs -
VAR INT MOBSIDIALOG;
CONST INT NONE=0;
CONST INT BEDDIALOG=1;
CONST INT RECEPTDIALOG=2;
CONST INT PRODUCTIONDIALOG=3;
//amunition
CONST INT FIREARROWDIALOG=4;
CONST INT ARROWDIALOG=5;
CONST INT ICEARROWDIALOG=6;
CONST INT BLUNTARROWDIALOG=7;
CONST INT ARROW_15_DIALOG=8;
CONST INT ARROW_25_DIALOG=9;
CONST INT POISONARROWDIALOG=10;
CONST INT FIREBOLTDIALOG=11;
CONST INT BOLTDIALOG=12;
CONST INT ICEBOLTDIALOG=13;
CONST INT BLUNTBOLTDIALOG=14;
CONST INT BOLT_10_DIALOG=15;
CONST INT BOLT_25_DIALOG=16;
CONST INT POISONBOLTDIALOG=17;
//runeswd
CONST INT RUNESWD1HDIALOG=18;//free
CONST INT RUNESWD2HDIALOG=19;//free
CONST INT DUALSWDDIALOG=20;
