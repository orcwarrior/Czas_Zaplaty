...
class zCAIPlayer {
    ...
    */
    ...
    var int vob;                    //zCVob*
    var int model;                  //zCModel*   0x068                    
    var int world;                  //zCWorld*                       
    
    var int centerPos[3];           //zPOINT3	
    var int feetY;                  //zREAL    0x07C                   
    var int headY;                  //zREAL    0x080                    
    var int aboveFloor;             //zREAL    0x084                    
    
    var int waterLevel;             //int //0: niedrig genug, 1: schon am Waten, 2: am Schwimmen
    
    var int velocityLen2;           //zREAL   0x08C                     
    var int velocity[3];            //zVEC3   0x090-9C                     
    
    var int fallDownDistanceY;      //zREAL   0x0A0                     
...
const int oCAiHuman_bitfield_canEnableNextCombo = ((1 <<  1) - 1) <<  0;
const int oCAiHuman_bitfield_endCombo           = ((1 <<  1) - 1) <<  1;
const int oCAiHuman_bitfield_comboCanHit      	= ((1 <<  1) - 1) <<  2;
const int oCAiHuman_bitfield_hitPosUsed         = ((1 <<  1) - 1) <<  3;
const int oCAiHuman_bitfield_hitGraphical       = ((1 <<  1) - 1) <<  4;
const int oCAiHuman_bitfield_canDoCollisionFX   = ((1 <<  1) - 1) <<  5;
class oCAniCtrl_Human {
    ...
    var int npc;                             //oCNpc*   //0x12C              
    var int targetVob;                       //zCVob*   //0x140         
    ...
    // Walkmode     
    var int walkmode;                        //int     0x158                  
    var int nextwalkmode;                    //int     0x15C                  
    var int lastRunWalkSneak;                //int     0x160                  
    var int always_walk;                     //zBOOL   0x16C                 
    var int do_jump;                         //zBOOL   0x170                 
    var int defaultSurfaceAlign;             //int     0x174                  
    var int autoRollDirection;               //zREAL   0x178                 
    
    // Kopfbewegungen
    var int lookTargetx;                     //zREAL   0x17C                 
    var int lookTargety;                     //zREAL   0x180               
    
    // Fight        
    var int distance;                           //int                        
    var int hitpos[3];                          //zVEC3    //0x188             
    var string limbname;                        //zSTRING   //0x194              
    var int paradeBeginFrame;                   //zREAL     //0x1A8            
    var int paradeEndFrame;                     //zREAL     //0x1AC            
    ...
    var int bitfield; //so wie im Struct im Kommentar   //0x1B0
    
    var int comboNr;           //int                  
    var int comboMax;          //int                  
    var int lastHitAniFrame;   //zREAL  0x1BC        
    var int hitAniID;          //zTModelAniID 0x1C0
    var int hitTarget;         //zCVob*   0x1C4      

    /*
    typedef struct {
        zTModelAniID    hitAni;                             
        int             hitOptFrame; [1] + n * 6
        int             hitEndFrame; [2] + n * 6
        int             comboStartFrame;
        int             comboEndFrame;
        int             comboDir;
    } oTComboInfo;*/
    ...
    // Kampfsystem - Tuning
    var int anioffset_lastper;  //zREAL   0x2B8               
    var int anioffset_thisper;  //zREAL   0x2BC              
    var int anioffset;          //zREAL   0x2C0               
    var int anioffset_ani;      //zTModelAniID  0x2C4     
